...
class Root(Tk):
    def __init__(self):
        ...
        self.image_model = pipeline("image-classification", model="google/vit-base-patch16-224")
        ...
        drop_label = Label(banner, text="Model Selection:", style='banner.TLabel')
        ...
        dropdown = OptionMenu(banner, self.selected_option, options[0], *options)
        dropdown.grid(row=0, column=1, padx=10, pady=10, sticky="w")

        button = Button(banner, text="Load Model", command=self.on_button_click, style='banner.TButton')
        button.grid(row=0, column=2, padx=10, pady=10, sticky="e")

        # ------------------------ In_Frame
        in_frame = LabelFrame(container, text="User Input Section", style='basic.TLabelframe')
        ...
        radio_frame = LabelFrame(in_frame, text='Input Type', style='mini.TLabelframe')
        radio_frame.grid(row=0, column=0, rowspan=2, sticky='nsew', padx=10, pady=(5, 0))
        ...
        text_radio.grid(row=0, column=0, sticky="nwe", padx=(2, 0), pady=(2, 0))

        image_radio = Radiobutton(radio_frame, text='Image', value="Image", variable=self.radio_var,
                                  command=self.update_input_mode, style='basic.TRadiobutton')
        image_radio.grid(row=1, column=0, sticky="swe", padx=(2, 0), pady=(0, 2))

        self.browse_btn = Button(in_frame, text="Browse", command=self.browse, style='basic.TButton')
        ...
        self.in_box.grid(row=0, column=1, rowspan=4, sticky="nsew", padx=(0, 10), pady=5)

        self.preview_label = Label(in_frame, text='Image Preview', style='basic.TLabel')
        self.preview_label.config(anchor='center', relief='solid', font=('Arial', 12, 'bold'),
                                  background='lightgray', foreground='darkgrey')
        self.preview_label.grid(row=0, column=2, rowspan=3, sticky='nsew', padx=(0, 10), pady=5)
        ...
        # ---------------------- out_frame
        out_frame = LabelFrame(container, text="Model Output Section", style='basic.TLabelframe')
        out_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=(5, 10))
        out_frame.grid_rowconfigure(1, weight=1)
        out_frame.grid_columnconfigure(0, weight=1)

        out_label = Label(out_frame, text='Output Display:', style='basic.TLabel')
        ...
        # ----------------------------- place holder
        place_holder = LabelFrame(container, text='placeholder -- (will be substituted)')
        place_holder.grid(row=2, column=1, sticky='nes', padx=10, pady=(5, 10))

        out_box2 = Text(place_holder, height=10, wrap='word')
        out_box2.grid(row=1, column=0, sticky="nw", padx=10, pady=10)
    ...
    def update_input_mode(self):
        ...
        if self.radio_var.get() == "Image":
            ...
            self.preview_label.config(background='lightgray', foreground='darkgray')
